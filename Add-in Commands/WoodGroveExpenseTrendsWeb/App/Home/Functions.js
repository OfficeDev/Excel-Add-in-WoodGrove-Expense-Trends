    ...
    Excel.run(function (ctx) {
        ...
        return ctx.sync()
            .then(function () {
                ...
                // Queue commands to protect the sheet from user interaction
    ...
    Excel.run(function (ctx) {
        ...
        return ctx.sync()
                   .then(function () {
                       ...
                       var masterTableAddress = 'Transactions!B20:G' + (startRowNumber + myBankTransactions.length).toString();
                       ...
                       for (var i in myBankTransactions) {
                           ...
                           tableBodyData.push([myBankTransactions[i].DATE, myBankTransactions[i].AMOUNT, myBankTransactions[i].MERCHANT, myBankTransactions[i].CATEGORY, '=IF(OR((TEXT([DATE], "dddd") = "Saturday"), (TEXT([DATE], "dddd") = "Sunday")), "Weekend", "Weekday")', '=TEXT([DATE], "mmm - yyyy")']);
                       }

                       // Queue a command to write the data to the table
                       ...
                       addContentToWorksheet(dataSheet, "B21:G" + (startRowNumber + myBankTransactions.length).toString(), "", "TableDataRows");
                       ...
                       //Queue commands to add the new chart and format it
...
function createDashboard() {
    ...
    Excel.run(function (ctx) {
        ...
        // Queue a command to add the Expenses by Organization table
...
function createDonationsTracker() {
    ...
    Excel.run(function (ctx) {
        ...
        donationsSheet.getRange("A2:R250").format.fill.color = "white";
        ...
        // Run the queued-up commands, and return a promise to indicate task completion
        return ctx.sync            // Queue a command to add a new worksheet to track the donations
        var donationsSheet = ctx.workbook.worksheets.add("Donations");
...
function addContentToWorksheet(sheetObject, rangeAddress, displayText, typeOfText) {
    ...
    switch (typeOfText) {
        case "SheetTitle":
            ...
            sheetObject.getRange("A1:R1").format.fill.color = "#41AEBD";
            break;
        case "SheetHeading":
            var range = sheetObject.getRange(rangeAddress);
            range.values = displayText;
